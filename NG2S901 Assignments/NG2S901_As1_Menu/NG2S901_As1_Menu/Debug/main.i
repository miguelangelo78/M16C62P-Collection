#line 1 "C:/Users/Miguel/Desktop/Nova pasta/NG2S901_As1_Menu/NG2S901_As1_Menu/main.c"
...
#line 2 "C:/Users/Miguel/Desktop/Nova pasta/NG2S901_As1_Menu/NG2S901_As1_Menu/main.c"
...
menu_option_t * menu_option_create(char * option_name, program_t cback) {
	...
	new_option->submenu = 0;  
	new_option->prog_callback = cback ? cback : 0;  
	return new_option;  
}
...
menu_option_t ** create_options(int option_count, ...) {
	...
	(args = ((va_list)&option_count + sizeof(option_count))) ;
	for(i = 0; i < option_count; i++) {  
		char * str = (args += sizeof(char*), *((char* *)(args - sizeof(char*)))) ;  
		program_t cback = (args += sizeof(program_t), *((program_t *)(args - sizeof(program_t)))) ;  
...
void animate_menu_name(menu_t * current_menu, int offset) {
	int i;
	GLCD_GoTo(0, 0);
	for(i = 0; i <= ((124/6) - strlen(current_menu->menu_name)) / 2 - (offset + 1); i++)
		GLCD_WriteChar('*', Font_System5x8) ;
	printf(" %s ", current_menu->menu_name);
	while(1)
		if(GLCD_getXY().x <= 124 - 2)
			GLCD_WriteChar('*', Font_System5x8) ;
		else 
			break;
	GLCD_GoTo(0, 2);
...
void menu_display_options(menu_t * current_menu) {
	...
	for(i = 0; i < current_menu->options_count; i++) {
		...
		if(!option->submenu && !option->prog_callback) continue;  
		printf(" %d- %s\n", i+1, option->option_name);  
		p = GLCD_getXY();
		 
		if(option->submenu) {  
			GLCD_GoTo(124 -19, p.y-1);
			GLCD_WriteString("...", Font_System5x8) ;	
		} else {  	
			GLCD_GoTo(124 -8, p.y-1);
			GLCD_WriteString(">", Font_System5x8) ;	
		}
		GLCD_GoTo(p.x, p.y);
	}
...
void menu_navigate(void) {  
	...
	int ret = 0;
	char update_menu = 0;
	char menu_name_offset_dir = 0;  
	char key;
	int menu_name_offset = 0, delay = 0;  
	int menu_name_bounds = 0;  
	for(;;) {
		 
		GLCD_ClearScreen(); GLCD_GoTo(0, 0);
		 
		animate_menu_name(current_menu, menu_name_offset);
		menu_name_bounds = 10-strlen(current_menu->menu_name);  
		 
		menu_display_options(current_menu);
		 
		if(current_menu->parent_menu) printf_at_rl("%c-Back", 124 - 7, 8 - 2, BACK);
		 
		if(ret) { printf_at("ret=%d", 3, 8 -2, ret); ret = 0; }
		 
		printf_at("_____________________", 0, 8 -1);
				...
				default: 
					if(key >= '1' && key <= '9') {
						...
						if(key >= 0 && key < current_menu->options_count && 
							...
							if(current_menu->options[key]->prog_callback) {  
								GLCD_ClearScreen();
								p = GLCD_GoTo(0, 0);
...
void init_menu_tree(void) {
	...
	menu_root = menu_create(0, 0, create_options(5, "GPIO", (program_t)0 , "GLCD & Keypad", (program_t)0 , "Timers", (program_t)0 , "Info", info, "Extra", extra));
